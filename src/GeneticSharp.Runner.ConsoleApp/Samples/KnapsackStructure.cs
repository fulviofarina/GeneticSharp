...
namespace GeneticSharp.Runner.ConsoleApp.Samples
{
    ...
    public class Knap
    {
        ...
        public string VolumenString = string.Empty;
        public string WeightString = string.Empty;
        public string ValueString = string.Empty;

        public void SetStrings(List<double> m_values, List<double> m_weights, List<double> m_volumes)
        {
            ...
            Console.WriteLine("Genotype:\t\t {0}", text);
            Console.WriteLine();

            ValueString = DecodeStrings(m_values.Cast<object>());
            WeightString = DecodeStrings(m_weights.Cast<object>());
            VolumenString = DecodeStrings(m_volumes.Cast<object>());
